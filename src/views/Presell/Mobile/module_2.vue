  ...
  <div class="presell-module_2">
    <div class="purchase">
      <div class="data-transform" :style="{ order: !isTransform ? 1 : 3 }">
        <p>
          <span>{{ isTransform ? $t('purchaseAndPledge.to') : $t('purchaseAndPledge.from') }}</span>
          <span>
            {{$t('purchaseAndPledge.quantity')}}：1999{{$t('global.busd')}}({{ isTransform ? "1TOTO=0.1BUSD" : "1BUSD=10TOTO" }})
          </span>
        </p>
        <div>
      ...
      <div class="data-transform" :style="{ order: !isTransform ? 3 : 1 }">
        ...
        <div>
          ...
          <div>
            <input type="number" :placeholder="0" :value="10000" />
          </div>
        </div>
      </div>
      <button style="order: 4;">{{ $t('purchaseAndPledge.purchase') }}</button>
    </div>
