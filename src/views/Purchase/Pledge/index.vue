  ...
  <div class="two_page mobile">
    <div class="nav">
      ...
      <ul class="two_page_nav">
        ...
        <li>
          <router-link to="/purchase/extract">{{ $t("purchaseAndPledge.withdrawal") }}</router-link>
        </li>
      </ul>
    </div>
    <div class="content">
      <div class="pledage_w">
        <p class="pledage_top">OZC{{ $t("purchaseAndPledge.quantity") }}：{{ ozcBalance || 0 }}</p>
        <div class="pledage_input_w">
          <p>¥</p>
          <input
            v-model="pledgeNumber"
            type="number"
            :placeholder="$t('tipMessage.num')"
          />
          <span @click="allPledge" class="all">{{ $t('global.all') }}</span>
        </div>
        <div class="pledage_data">
          <p>{{ $t("obtain") }}：{{ pledgeNumber || 0 }} {{ $t('global.ozc') }}</p>
          <!-- <p>{{ $t('预计获得TOTO') }}：未对接TOTO</p> -->
          <p>{{ $t("purchaseAndPledge.annualizedReturn") }}：{{ this.stakeRate }}%</p>
        </div>
      </div>
      <div class="pledage_btn">
        <button @click="pledgeBtn">{{ $t("global.submit") }}</button>
        <p>{{ $t("purchaseAndPledge.obtain") }}{{ $t("global.ozc") }}{{ $t("purchaseAndPledge.stake") }}{{ $t("global.toto") }}</p>
      </div>
    </div>
    <Hint ref="hint" />
  </div>
